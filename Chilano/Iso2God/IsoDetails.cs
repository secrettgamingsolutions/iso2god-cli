    ...
    internal class IsoDetails
    {
        ...
        private IsoDetailsResults readXex()
        {
            ...
            using (XexInfo info = new XexInfo(bytes))
            {
                ...
                // Skip XEX scanning - compressed XEX files don't contain readable XDBF data
                // Go directly to online API lookup
                Console.WriteLine("+ Looking up game title online (TitleID: " + results.TitleID + ")...");
                string gameName = TitleLookupService.LookupTitleByTitleId(results.TitleID);
